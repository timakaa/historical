...
func NewClient(address string) (*Client, error) {
	...
	client := pb.NewHistoricalPricesClient(conn)
	return &Client{
		conn:   conn,
		client: client,
	}, nil
...
func (c *Client) GetHistoricalPrices(exchange string, ticker string) ([]*pb.HistoricalPricesResponse, error) {
	ctx, cancel := context.WithTimeout(context.Background(), time.Second*10)
	defer cancel()

	// Create request
	req := &pb.HistoricalPricesRequest{
		Exchange: exchange,
		Ticker:   ticker,
	}

	// Get stream
	stream, err := c.client.GetHistoricalPrices(ctx, req)
	if err != nil {
